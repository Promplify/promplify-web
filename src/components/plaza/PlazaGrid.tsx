...
export function PlazaGrid({ prompts, isLoading, currentPage, totalItems, itemsPerPage, onPageChange }: PlazaGridProps) {
  ...
  return (
    <div>
      <div id="all-prompts" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {prompts.map((prompt) => (
          <PlazaCard key={prompt.id} plazaPrompt={prompt} />
        ))}
      </div>
